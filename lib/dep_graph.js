  ...
  overallOrder:function (leavesOnly) {
    ...
    if (keys.length === 0) {
      ...
      keys.filter(function (node) {
        // We are trying to find a node with zero incoming edges
        // so we have a starting point for our overall order
        return self.incomingEdges[node].length === 0;
      }).forEach(function (n) {
        DFS(n);
      });
      if (result.length === 0) {
        // There's definitely a cycle somewhere - run a fresh DFS on every node till
        // one generates the cyclic error
        keys.forEach(function(node){
          createDFS(self.outgoingEdges, false, [])(node);
        });
        // Do not rely on DFS to throw errors
        throw new Error('Could not find a node without any dependents to start from');
      }
      return result;
    }
  }
